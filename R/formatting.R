...
#' x <- 3e-4
#' signif_sym(x)
signif_sym <- function(x, max_asterisks=5, bonferoni = 1, NA_instead_of_error = F, include_dot = T, ns_sym = ' ') {
  if(!NA_instead_of_error){
    if(any(x<0) | any(x>1)){
      stop("Error in signif_sym(...), passed probabilities cannot be smaller than 0 or bigger than 1 if NA_instead_of_error param is set to FALSE.")
    }
  }
  if(max_asterisks<2){
  ...
  cutpoints = c(0, cutpoints, 0.05, ifelse(include_dot, 0.1, NULL), 1)
  symbols = sapply(max_asterisks:1, function(x){paste(rep('*', x), collapse='')})
  symbols = c(symbols,ifelse(include_dot, '.', NULL), ns_sym)
